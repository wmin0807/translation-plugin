...
一个轻量、无自建后端的 Chrome / Edge 网页翻译扩展。

在网页中快速连续按两次物理 `F` 键即可翻译：有选中文字时，译文会在原文附近流式显示；没有选中文字时，会弹出输入框供你输入或粘贴内容。扩展会直接调用你配置的大模型 API。

当前版本：**v0.3.0**。手动输入单个英文单词时，显示原单词、英式/美式 IPA 音标和词性释义。详细变化见 [更新日志](CHANGELOG.md)。
...
1. 确保网页中没有选中文字。
2. 在 700ms 内连续按两次物理 `F` 键。
3. 在页面中央的输入框中输入或粘贴要翻译的内容。
4. 按 `Cmd + Enter`（macOS）或 `Ctrl + Enter`（Windows / Linux），也可以点击**翻译**按钮。
5. 译文会在同一弹窗内流式显示；你可以复制结果、修改原文后再次翻译，或关闭弹窗。

输入单个英文单词（例如 `apple`）后，结果会显示单词、英式/美式 IPA 音标和词性释义。复制结果会包含单词、音标和释义。

单词模式支持英文大小写、内部撇号和连字符；短语、句子及中文仍按普通文本翻译。音标由当前配置的模型生成，无法确定时会提示不可用。

如果从网页原有输入框中触发，扩展会清理快捷键产生的多余 `f` 字符；关闭翻译弹窗后，会恢复原输入框的焦点和光标位置。
...
- 只有在你选中文字并主动按下 `FF`，或在手动输入框中提交翻译后，相应文字才会发送给配置的模型服务商。
